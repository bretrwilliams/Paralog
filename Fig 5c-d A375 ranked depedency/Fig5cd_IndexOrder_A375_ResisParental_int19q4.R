...
ceres <- load.from.taiga(data.name='internal-19q4-fb11', data.version=22, data.file='Achilles_gene_effect')
cl3c <- ceres[celllinemapr::ccle.to.arxspan(c("A375_SKIN","A375_SKIN_CJ1_RESISTANT","A375_SKIN_CJ3_RESISTANT")),]%>%
  t()%>%
  as.data.frame()%>%
  tibble::rownames_to_column("gene")%>%
  magrittr::set_colnames(c("G","A375_SKIN","A375_SKIN_CJ1_RESISTANT","A375_SKIN_CJ3_RESISTANT"))%>%
  mutate(gene = gsub(" \\(.*","",G))%>%
  select(-G)%>%
...
ggplot(D, aes(x=index, y=diff,color=note)) + 
  ...
  geom_text_repel(data = D%>%group_by(situ)%>%arrange(diff)%>%slice(1:5)%>%
            ...
            fontface = 'bold', 
            family = 'Helvetica')
ggsave(width = 10,height=5,filename = "Fig 5c-d A375 ranked depedency/Fig5cd_A375_Delta_Ceres_scatter_19q4_internal.pdf",dpi = 2000)
