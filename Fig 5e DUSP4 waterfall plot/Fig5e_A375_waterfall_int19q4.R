...
mut_annot <- mut[,-1]%>%
  select(Hugo_Symbol,isTCGAhotspot,DepMap_ID)%>%
  filter(Hugo_Symbol %in% c("BRAF") & isTCGAhotspot=="True")
Mut_ceres <- ceres%>%
  as.data.frame()%>%
  ...
  mutate(Mutations = case_when(is.na(isTCGAhotspot)~"WT",TRUE~Hugo_Symbol))
...
ggsave(w,width = 5,height=5,filename = paste0("Fig 5e DUSP4 waterfall plot/","Fig5e_A375_ceres_waterfall_int19q4_",Sys.Date(),".pdf"),dpi = 2000)
